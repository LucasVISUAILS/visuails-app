...
- **Services** — de 3 servicepagina's (Catalog, Lifestyle, Video): tekst, "how it works"-stappen,
  en per pagina alle losse **styles** (bijv. Classic/Custom bij Catalog) met eigen prijs, features,
  afbeelding en FAQ's
- **Site Settings** — bedrijfsnaam, e-mail, WhatsApp-nummer, adres, openingstijden, KVK/BTW,
  social links, de "About"-tekst en de statistieken-balk op de homepage
...
**Niet (bewust uitgezet):** nieuwe servicepagina's toevoegen of verwijderen via de admin — dat kan
per ongeluk de site breken (links, sitemap, schema die naar een pagina wijzen die niet meer
bestaat). Losse **styles** binnen een service (bijv. een 4e Catalog-stijl) kun je wel gewoon zelf
toevoegen via de lijst-widget in `/admin`. Wil je een hele nieuwe servicepagina? Laat het me weten,
dan bouw ik die erbij — kost me een paar minuten.
